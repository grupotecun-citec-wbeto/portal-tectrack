...
function CardAsistencia(props) {
    ...
    return (
        <Card>
            <CardHeader>
                <Heading as="h2" fontSize={{ base: "md", md: "lg" }} fontWeight="semibold">¿Que tipo asistencia consideras que se dará?</Heading>
            </CardHeader>
            <CardBody mt='20px'>
                <Flex ms={{ xl: '25px', sm: '25px' }}>
                    <FormControl maxW={{ xl: '250px' }}>
                        <FormLabel htmlFor='country' fontSize={{ base: "sm", md: "md" }}>Tipo de Asistencia</FormLabel>
                        <Select id='country' placeholder='Seleccionar tipo de asistencia' onChange={(e) => actionSelectAsistencia(e.target.value)} value={selectedAsistencia} fontSize={{ base: "sm", md: "md" }}>
