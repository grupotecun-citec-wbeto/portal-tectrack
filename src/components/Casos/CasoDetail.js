...
const CasoDetail = React.memo(({ caseData, openLoader }) => {
  ...
  // obtener los predianosticos
  const { 
    data: prediagnosticoData, 
    loading: loadingPrediagnostico, 
    error: errorPrediagnostico 
  } = useGetPrediagnosticosByCasoId(id,status_ID,dbReady);
  ...
  useEffect(() => {
    //data: prediagnosticoData, loading:loadingPrediagnostico, error:errorPrediagnostico
    if (!loadingPrediagnostico) {
      setCantEquipos(prediagnosticoData?.length)
    } else {
      setCantEquipos(0)
    }


  }, [prediagnosticoData, loadingPrediagnostico])
