/**
 * @package repositories/local/unidad_negocio
 * @description Repositorio de la tabla de unidad_negocio
 * @author CITEC
 */

const PACKAGE = 'repositories/local/unidad_negocio';
...
const repository = {
    ...
    createOrRemplace: async (json) => {
        const db = getDB();
        const stmt = db.prepare(`INSERT OR REPLACE INTO ${repository.tableName} (ID, nombre, unidad_negocio_ID) VALUES (?, ?, ?)`);

        for (const {ID, nombre, unidad_negocio_ID} of json) {
            stmt.run([ID, nombre, unidad_negocio_ID]);
        }
        stmt.free();
