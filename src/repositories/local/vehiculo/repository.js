/**
 * @package repositories/local/vehiculo
 * @description Repositorio de la tabla de vehiculo
 * @author CITEC
 */

const PACKAGE = 'repositories/local/vehiculo';
...
const repository = {
    ...
    createOrRemplace: async (json) => {
        const db = getDB();
        const stmt = db.prepare(`INSERT OR REPLACE INTO ${repository.tableName} (ID,code,placa,year,name) VALUES (?, ?, ?, ?, ?)`);

        for (const {ID,code,placa,year,name} of json) {
            stmt.run([ID,code,placa,year,name]);
        }
        stmt.free();
