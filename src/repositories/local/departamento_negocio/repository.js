 ...
 * @description Repositorio de la tabla de departamento_negocio
 * @author CITEC
 */
...
const repository = {
    tableCode:11,
    tableName:'departamento_negocio',
    /**
     * 
    ...
    createOrRemplace: async (json) => {
        const db = getDB();
        const stmt = db.prepare(`INSERT OR REPLACE INTO ${repository.tableName} (ID, name, unidad_negocio_ID) VALUES (?, ?, ?)`);

        for (const {ID, name, unidad_negocio_ID} of json) {
            stmt.run([ID, name, unidad_negocio_ID]);
        }
        stmt.free();
